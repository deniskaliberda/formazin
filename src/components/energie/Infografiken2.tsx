  ...
  return (
    <svg viewBox="0 0 640 300" role="img" aria-label="Der iSFP-Bonus als Hebel: Fördersatz für BAFA-Einzelmaßnahmen steigt von 15 auf 20 Prozent, der förderfähige Höchstbetrag von 30.000 auf 60.000 Euro je Wohneinheit und Jahr." className="h-auto w-full">
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        <g fontSize="12.5" fontWeight="700" fill={TINTE_60} textAnchor="middle" letterSpacing="0.08em">
        ...
        <text x="510" y="171" fontSize="11" fill={TINTE_60} fontFamily={BODY}>Fördersatz (BAFA)</text>
        {/* Höchstbetrag */}
        ...
        <text x="510" y="111" fontSize="11" fill={TINTE_60} fontFamily={BODY}>Höchstbetrag je WE und Jahr</text>
        <text x="0" y="284" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>Gilt für BAFA-Einzelmaßnahmen an Gebäudehülle und Anlagentechnik, nicht für den Heizungstausch (KfW 458).</text>
      </g>
    </svg>
  );
}
      ...
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        {cells.map((i) => (
          <g key={i}>
            ...
            <text x={x0 + i * w + 8} y={y0 + 18} fontSize="12.5" fontWeight="700" fill={TINTE_60}>Woche {i + 1}</text>
          </g>
        ))}
        {/* Ereignisse (zweizeilig, damit nichts über die Wochenspalte läuft) */}
        <rect x={x0 + 3} y={y0 + 30} width={w - 10} height="38" fill={BLAU} />
        <text x={x0 + 8} y={y0 + 46} fontSize="10.5" fill="#fff" fontWeight="700">Erstgespräch</text>
        <text x={x0 + 8} y={y0 + 61} fontSize="10.5" fill="#fff">20 min, gratis</text>
        <rect x={x0 + w + 6} y={y0 + 74} width={2 * w - 16} height="38" fill={BLAU_HELL} />
        <text x={x0 + w + 12} y={y0 + 90} fontSize="11.5" fontWeight="700">Vor-Ort-Termin, 1–2 h</text>
        <text x={x0 + w + 12} y={y0 + 105} fontSize="11">nach 7–14 Tagen</text>
        <rect x={x0 + 2 * w + 6} y={y0 + 118} width={4 * w - 16} height="26" fill={BLAU} />
        <text x={x0 + 2 * w + 12} y={y0 + 135} fontSize="11.5" fill="#fff" fontWeight="700">iSFP-Erstellung, 2–4 Wochen</text>
        <rect x={x0 + 6 * w + 3} y={y0 + 118} width={w - 10} height="26" fill={TINTE} />
        <text x={x0 + 6 * w + 8} y={y0 + 135} fontSize="10.5" fill="#fff" fontWeight="700">Gespräch 1 h</text>
        <text x="0" y="244" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>Typischer Verlauf. Wer danach saniert, hängt Antrag, Bauleitung und Verwendungsnachweis direkt an.</text>
      </g>
    </svg>
  );
}
...
const SAEULEN = [
  ...
  { t: "Formazin & Partner", items: ["Bestandsanalyse, iSFP", "Förderberatung, Anträge", "Planung, Bauantrag, Bauleitung", "Energieausweis, GEG-Nachweis", "Techn. + Verwendungsnachweis"], fill: BLAU, tf: "#fff" },
        ...
        {SAEULEN.map((s, i) => {
          ...
          return (
            <g key={s.t}>
              ...
              {s.items.map((it, j) => (
                <g key={it}>
                  <text x={x + 14} y={98 + j * 34} fontSize="13" fontWeight="700" fill={i === 1 ? "#fff" : BLAU}>+</text>
                  <text x={x + 28} y={98 + j * 34} fontSize="12" fontFamily={BODY} fill={s.tf}>{it}</text>
                </g>
              ))}
            </g>
          );
        ...
        {RECHNUNG.map((r, i) => {
          ...
          return (
            <g key={r.t}>
              ...
              <text x={x + 14} y="64" fontWeight="700" fontSize="13.5">{r.t}</text>
              <line x1={x + 14} y1="76" x2={x + W - 14} y2="76" stroke={HAIR} />
              <text x={x + 14} y="104" fontSize="13" fontFamily={BODY}>Honorar brutto</text>
              <text x={x + W - 14} y="104" textAnchor="end" fontWeight="700">{r.h}</text>
              <text x={x + 14} y="134" fontSize="13" fontFamily={BODY}>BAFA-Zuschuss</text>
              <text x={x + W - 14} y="134" textAnchor="end" fontWeight="700" fill={BLAU}>{r.z}</text>
              {r.n && <text x={x + 14} y="152" fontSize="12" fontFamily={BODY} fill={TINTE_60}>{r.n}</text>}
              <line x1={x + 14} y1="168" x2={x + W - 14} y2="168" stroke={TINTE} strokeWidth="2" />
              <text x={x + 14} y="200" fontWeight="700" fontSize="13.5">Ihr Eigenanteil</text>
              <text x={x + W - 14} y="204" textAnchor="end" fontWeight="700" fontSize="22">{r.e}</text>
              <text x={x + 14} y="240" fontSize="12" fontFamily={BODY} fill={TINTE_60}>50 % Zuschuss, gedeckelt.</text>
              <text x={x + 14} y="256" fontSize="12" fontFamily={BODY} fill={TINTE_60}>Antrag vor Beauftragung.</text>
            </g>
          );
        })}
      </g>
  ...
  return (
    <svg viewBox="0 0 640 260" role="img" aria-label="Ihr Zeitaufwand für einen Sanierungsfahrplan: 20 Minuten Erstgespräch, 1 bis 2 Stunden Vor-Ort-Termin, 1 Stunde Beratungsgespräch, zusammen rund drei bis vier Stunden. Unsere Bearbeitung: Vor-Ort-Termin in 7 bis 14 Tagen, iSFP-Erstellung 2 bis 4 Wochen." className="h-auto w-full">
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        <text x="0" y="56" fontSize="12.5" fontWeight="700" fill={TINTE_60} letterSpacing="0.08em">IHR AUFWAND</text>
        ...
        <text x="0" y="210" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>mehr müssen Sie nicht einplanen</text>
        <line x1="330" y1="46" x2="330" y2="220" stroke={HAIR} />
        <text x="350" y="56" fontSize="12.5" fontWeight="700" fill={TINTE_60} letterSpacing="0.08em">UNSERE BEARBEITUNG</text>
        ...
        <text x="350" y="210" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>bis der Fahrplan im BAFA-Format vorliegt</text>
        <text x="0" y="248" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>Erstgespräch kostenlos, telefonisch oder per Video.</text>
      </g>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 640 300" role="img" aria-label="KfW-Baubegleitung in fünf Schritten: Erstgespräch und Bestandsaufnahme, Sanierungsplan iSFP, KfW-Antrag mit Energieeffizienz-Experten-Bestätigung, Handwerker-Vergleich mit drei Angeboten pro Gewerk, Bauleitung mit Stichproben-Abnahme und Verwendungsnachweis. Honorar 3.500 Euro Einfamilienhaus, 4.500 Zweifamilienhaus, 6.500 Mehrfamilienhaus, förderfähig im Rahmen der BEG." className="h-auto w-full">
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        <line x1="60" y1="90" x2="580" y2="90" stroke={BLAU} strokeWidth="2" />
        {BB.map((s, i) => {
          const x = 60 + i * 130;
          return (
            <g key={s[0]}>
              <circle cx={x} cy="90" r="16" fill={i === 4 ? TINTE : BLAU} />
              <text x={x} y="95" textAnchor="middle" fill="#fff" fontWeight="700" fontSize="14">{i + 1}</text>
              <text x={x} y="130" textAnchor="middle" fontSize="12" fontWeight="700">{s[0]}</text>
              <text x={x} y="146" textAnchor="middle" fontSize="11.5" fontFamily={BODY} fill={TINTE_60}>{s[1]}</text>
            </g>
          );
        })}
        <rect x="0" y="180" width="640" height="1" fill={HAIR} />
        <text x="0" y="210" fontWeight="700">Honorar Baubegleitung</text>
        <text x="0" y="232" fontFamily={BODY} fontSize="13.5">3.500 € Einfamilienhaus · 4.500 € Zweifamilienhaus · 6.500 € Mehrfamilienhaus · Nichtwohngebäude individuell</text>
        <text x="0" y="256" fontFamily={BODY} fontSize="12.5" fill={TINTE_60}>Die Baubegleitung ist im Rahmen der BEG-Programme selbst förderfähig. Antrag immer vor Beauftragung.</text>
      </g>
    </svg>
  );
}
...
export function NachweisKette() {
  const steps = [
    ...
    { t: "Umsetzung", w: "Fachbetrieb + wir", fill: BLAU_TINT, tf: TINTE },
    ...
    <svg viewBox="0 0 640 220" role="img" aria-label="Die Nachweis-Kette: Antrag vor der Beauftragung, Umsetzung durch den Fachbetrieb mit unserer Begleitung, technischer Nachweis nach Fertigstellung, Verwendungsnachweis mit lückenlosen Belegen, dann die Auszahlung. Ohne Antrag vor Beauftragung entfällt der Förderanspruch." className="h-auto w-full">
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        {steps.map((s, i) => {
          ...
          return (
            <g key={s.t}>
              <path d={`M ${x} 50 h 112 l 12 22 l -12 22 h -112 z`} fill={s.fill} stroke={i === 1 ? BLAU : "none"} />
              {s.t === "Verwendungsnachweis" ? (
                <>
                  <text x={x + 8} y="70" fontSize="11" fontWeight="700" fill={s.tf}>Verwendungs-</text>
                  <text x={x + 8} y="84" fontSize="11" fontWeight="700" fill={s.tf}>nachweis</text>
                </>
              ) : (
                <text x={x + 8} y="77" fontSize="11" fontWeight="700" fill={s.tf}>{s.t}</text>
              )}
        ...
        <text x="0" y="196" fontFamily={BODY} fontSize="13.5">Erst beauftragen, dann beantragen. Dann ist die Förderung weg. Wir stellen die Anträge, bevor Sie unterschreiben.</text>
      </g>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 640 280" role="img" aria-label="GEG-Nachweis im Bauantrag: Entwurfsplanung, dann der GEG-Nachweis mit Wärmeschutznachweis, sommerlichem Wärmeschutz und Lüftungskonzept, dann Einreichung des Bauantrags und Genehmigung. Festpreise 900 Euro Einfamilienhaus, 1.100 Zweifamilienhaus, 2.300 Mehrfamilienhaus." className="h-auto w-full">
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        {[["Wärmeschutznachweis", "Gebäudehülle, U-Werte, Primärenergie"], ["Sommerlicher Wärmeschutz", "Überhitzung vermeiden, Verschattung"], ["Lüftungskonzept", "Feuchteschutz, Luftwechsel"]].map((p, i) => {
          ...
          return (
            <g key={p[0]}>
              ...
              <text x={x + 10} y="138" fontSize="13" fontWeight="700">{p[0]}</text>
              <text x={x + 10} y="158" fontSize="12" fontFamily={BODY} fill={TINTE_60}>{p[1]}</text>
            </g>
          );
        })}
        ...
        <text x="0" y="252" fontFamily={BODY} fontSize="13.5">900 € Einfamilienhaus · 1.100 € Zweifamilienhaus · 2.300 € Mehrfamilienhaus · Nichtwohngebäude nach Aufwand</text>
        <text x="0" y="272" fontFamily={BODY} fontSize="12.5" fill={TINTE_60}>Aus dem Büro, das auch den Bauantrag plant. Nachweispflicht ab zehn Prozent Bauteilerneuerung.</text>
      </g>
    </svg>
  );
}
        ...
        {PROG.map((p, i) => {
          ...
          return (
            <g key={p.t}>
              ...
              <text x={x + 14} y="64" fontWeight="700" fontSize="13.5">{p.t}</text>
              <text x={x + 14} y="80" fontSize="12" fontFamily={BODY} fill={TINTE_60}>{p.s}</text>
              <text x={x + 14} y="128" fontWeight="700" fontSize="28" fill={BLAU}>{p.big}</text>
              {p.l.map((l, j) => (
                <text key={l} x={x + 14} y={160 + j * 22} fontSize="12" fontFamily={BODY}>{l}</text>
              ))}
            </g>
          );
        })}
    ...
    <svg viewBox="0 0 640 300" role="img" aria-label="Bedarfsausweis gegenüber Verbrauchsausweis: Grundlage Berechnung versus Verbrauchsdaten, objektiv versus nutzerabhängig, Preis 900 bis 2.300 Euro versus 100 Euro, Dauer 5 bis 10 Werktage versus rund 3 Werktage, Pflicht bei Bauantrag vor 1.11.1977 mit weniger als 5 Wohnungen ohne Nachrüstung, Empfehlung Bedarfsausweis bei Verkauf." className="h-auto w-full">
      <g fontFamily={HEAD} fontSize="13" fill={TINTE}>
        ...
        {VGL.map((r, i) => {
          ...
          return (
            <g key={r[0]}>
              <text x="0" y={y} fontWeight="700" fontSize="13">{r[0]}</text>
              <text x="138" y={y} fontSize="12" fontFamily={BODY}>{r[1]}</text>
              <text x="398" y={y} fontSize="12" fontFamily={BODY}>{r[2]}</text>
        ...
        {b.map((g, i) => {
          ...
          return (
            <g key={g.t}>
              ...
              <text x={x + 40} y={200 - g.h - 35} textAnchor="middle" fill="#fff" fontWeight="700" fontSize="13">{g.p}</text>
              <text x={x + 40} y="222" textAnchor="middle" fontWeight="700" fontSize="13">{g.t}</text>
              <text x={x + 40} y="238" textAnchor="middle" fontSize="12" fontFamily={BODY} fill={TINTE_60}>{g.s}</text>
            </g>
          );
        })}
        <text x="0" y="272" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>Kriterien: Zustand von Hülle und Anlagentechnik · erreichbare Förderquote · Belegung und Bauablauf · Nachweise je Objekt. Schematisch.</text>
        <text x="0" y="290" fontSize="12.5" fill={TINTE_60} fontFamily={BODY}>Ergebnis: förderfähige Sanierungspfade mit Reihenfolge, Anträge und Nachweise gebündelt, verbindliches Angebot nach Erstprüfung.</text>
      </g>
    </svg>
  );
}
