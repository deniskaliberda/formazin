...
export function AblaufSteps({ steps, compact = false }: { steps: (ContentSub & { phaseLabel?: string })[]; compact?: boolean }) {
  return (
    <ol className={compact ? `mt-6 grid list-none gap-6 p-0 sm:grid-cols-2 ${steps.length === 5 ? "lg:grid-cols-5" : steps.length === 3 ? "lg:grid-cols-3" : "lg:grid-cols-4"}` : "mt-6 max-w-3xl list-none space-y-0 p-0"}>
      {steps.map((step, i) => (
        ...
        >
          <span
            className={`flex h-9 ${step.phaseLabel ? "w-fit px-3" : "w-9"} flex-shrink-0 items-center justify-center rounded-[2px] bg-[#2d4196] font-heading text-sm font-bold text-white`}
            aria-hidden={step.phaseLabel ? undefined : true}
          >
            {step.phaseLabel ?? i + 1}
          </span>
          <div>
